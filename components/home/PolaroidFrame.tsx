...
const PolaroidFrame: React.FC<PolaroidFrameProps> = ({
  ...
  return (
    <>
      <div
        className={`relative fade-in opacity-0 translate-y-8 ${className} ${videoSrc ? 'cursor-pointer' : ''}`}
      ...
      >
        ...
        <div className={`${bgColor} p-4 pt-4 pb-8 shadow-xl`} style={{ width }}>
          {/* Media placeholder */}
          <div className="relative aspect-video bg-slate-200 mb-4">
            {videoSrc ? (
              <>
                <video
        ...
        {/* Tape element */}
{/*         <div className={`absolute -top-3 ${rotation === "-3deg" ? "right-1/2" : "right-1/3"} bg-gradient-to-r from-gray-100/30 to-gray-100/70 w-16 h-8 ${rotation === "-3deg" ? "rotate-12" : "-rotate-12"}`}>
        </div> */}
